...
def Count(file):
    # 定义结果存储字典
    results = defaultdict(lambda: [0, 0, 0, 0, 0])  # 初始化每个ID的统计信息
    ...
    with open(file, mode='r', encoding='utf-8') as file2count:
        ...
        for row in csv_reader:
            ...
            if Platform_Type == 'lease_platform':
                ...
                if Return_Time == '9999' and Refund_Amount == Product_Amount:
                    results[Consumer_ID][1] += 1
                if Product_Amount > Payment_Amount:
                    results[Consumer_ID][3] += 1
                if Payment_Amount != 0 and Shipping_Time == '9999':
    ...
    with open(result_file_name, mode='w', encoding='utf-8', newline='') as file:
        ...
        # 写入表头
        csv_writer.writerow(['ID', 'Total_Count', 'Refund_Only_Count', 'Rental_Not_Returned_Count', 'Partial_Payment_After_Receipt_Count', 'Payment_Without_Delivery_Count'])
